...
import { getUserId} from "./test_names.js";
import { array_answers } from "./data_questions.js";

import { userAnswers,userAnswersDuration,questionStartTime,stateAnswers,counttry} from "./test_display.js";
...
async function storeQuizCheck() {
    ...
    const quizResultRef = db.collection(databasename).doc(userId);
...
async function storeQuizResults() {
    const resultCheck = await storeQuizCheck();
    ...
    const { quizResultRef, userId } = resultCheck;
    ...
    try {
        ...
        let quizResponses = array_answers.map((item, index) => ({
            ...
            duration: userAnswersDuration[index] || 0
        }));
        ...
        console.log("Saving to Firestore:", resultData); // ✅ Debugging
        await quizResultRef.set({ quizResults: resultData }, { merge: true });
        console.log("Data successfully saved!"); // ✅ Debugging
...
async function saveProgress() {
  try {
    const savingMessage = showMessage("جاري الحفظ", "primary");
      ...
      const quizResultRef = db.collection(databasename).doc(userId); // Reference to user document
